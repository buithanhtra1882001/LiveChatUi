...
>((props, ref) => {
  ...
  const msgRefCus = useRef<HTMLDivElement>(null);
    ...
    throttle((el: HTMLElement) => {
      if (isNearBottom(el, 3)) {
        if (newCountRef.current) {
          // Nếu có tin nhắn mới, cách đáy 0.5 màn hình - ẩn thông báo
        ...
        // 3 màn hình trở lên - hiển thị nút quay về đáy
  ...
  useEffect(() => {
    ...
    if (lastMessage.position === "right") {
      // Tin nhắn do chính người dùng gửi, tự động cuộn xuống đáy
  ...
  return (
    <div className="MessageContainer" ref={messagesRef} tabIndex={-1}>
      ...
      <PullToRefresh
        ...
        ref={scrollerRef}
      >
        <div className="MessageList" ref={msgRefCus}>
          {messages.map((msg) => (
            <Message
              {...msg}
